...
const Dashboard = () => {
  ...
  const handleCreateNew = () => {
    // Calculo de Créditos Disponíveis para validação
    ...
    // Créditos do plano só valem se não estiver expirado
    const planRemaining = isExpired ? 0 : Math.max(0, planCredits - planUsed);

    // Regra de Ouro: Pode criar se tiver AVULSO (>0) OU PLANO VÁLIDO (>0)
    const canProceed = extraCredits > 0 || planRemaining > 0;

    if (!isAdmin && !canProceed) {
      toast.error('Você não possui créditos disponíveis. Renove seu plano ou adquira créditos avulsos.');
      return;
    }
            ...
            {subLoading ? (
              ...
              <>
                <div className="space-y-6">
                  ...
                  {/* Progress Bar - Only shows PLAN usage, not extra */}
                  {!isAdmin && subscription && (
                    <div className="h-2 w-full overflow-hidden rounded-full bg-muted">
                      <div
                        className="h-full bg-primary transition-all duration-300"
                        style={{
                          width: `${Math.min((((subscription.relatorios_usados || 0) / (subscription.relatorios_disponiveis || 1)) * 100), 100)}%`
                        }}
                      />
                    </div>
                  )}
